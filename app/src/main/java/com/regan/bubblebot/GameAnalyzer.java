...
/**
Fast local vision + geometry player tuned for the
588x1280 portrait bubble game.
 */
public final class GameAnalyzer {
 ...
 synchronized (LOCK) {
     ...
     if (SystemClock.uptimeMillis() - lastShot < 1200) {
         return;
     }
     ...
     if (current == Color.UNKNOWN || balls.size() < 3) {
         return;
     }


     Candidate best = chooseShot(
             balls,
             current
     );
  ...
  * FIX:
  *
  * The OpenCV version used by the project does not expose
  * COLOR_RGBA2HSV in the Java API.
  *
  * Convert:
  *
  * RGBA -> RGB -> HSV
 ...
 /*
  * Every matching ball gets six possible
  * neighboring landing cells.
  */
 for (Ball t : balls) {
 ...
 /*
  * Also consider shooting toward the centers
  * of matching balls.
  */
 for (Ball t : balls) {
 ...
 if (best.score < 20) {
     ...
     /*
      * A useful miss is preferable to firing
      * into an occupied/unsafe path.
      */
     return best;
 }
  ...
  *
  * Mirroring the target gives the
  * required initial ray.
  */
 double[] mirrors = {
         -tx,
         2 * CW - tx
 ...
 /*
  * dispatchGesture uses real display coordinates.
  * Aim at a point high in the board.
  */
 double len = 500;
 ...
 /*
  * For a left/right bank, a long swipe
  * is unnecessary: a tap at the aim point
